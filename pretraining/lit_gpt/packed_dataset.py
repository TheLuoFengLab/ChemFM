...
class PackedDatasetIterator:
    ...
    def __next__(self):
        ...
        # Feiyang: revise for fitting the molecule generation task
        arr = torch.from_numpy(arr.astype(np.int64))
        ### input:  <tok1><tok2><tok3>...<tokN><eos><MASKED>
        ### output: <tok2><tok3><tok4>....<eos><MASKED><MASKED>
        input_ids = arr[0 : -1]
        targets = arr[1:].clone() # Feiyang: must clone, otherwise the targets will be changed when input_ids is changed
        input_ids[input_ids < 0] = 265#155 # Feiyang: 155 is the id for <eos>
        ### input:  <tok1><tok2><tok3>...<tokN><eos><eos>
        ### output: <tok2><tok3><tok4>....<eos><MASKED><MASKED>
